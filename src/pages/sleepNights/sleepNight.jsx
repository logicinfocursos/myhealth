...
export default function () {
    ...
    const fetchdata = async () => {
        ...
        const result = await apiGET(`sleepNights/${id}`)

        setSleepNight(result.data)
    ...
    return (
        ...
        <div className="container">
            ...
            <div className="alert alert-danger mb-3 mt-3" role="alert" style={{ display: "none" }} id="deleteMessage">
                <h4 className="alert-heading">excluir registro #{sleepNight.id}</h4>
            ...
            <div className="card mt-5 mb-5">
                ...
                <div className="card-header">
                    horário de sono {sleepNight.id ? '(# ' + sleepNight.id + ")" : ''}
                </div>
...
export const submitForm = (event, sleepNight, setSleepNight) => {
    ...
    if (fieldsVerify(sleepNight)) {
        ...
        const _sleepNight = {
            ...
            status: 1,
            created_at: sleepNight.created_at ? sleepNight.created_at : getDateTime(),
            updated_at: sleepNight.updated_at ? sleepNight.updated_at : getDateTime(),
        }

        setSleepNight(_sleepNight)

        if (sleepNight.id && sleepNight.id > 0) _sleepNight.id = sleepNight.id

        if (_sleepNight.id && _sleepNight.id > 0) updateData(_sleepNight)
...
export const addData = async (_sleepNight) => {
    ...
    const result = await apiPOST('sleepNights', _sleepNight)
...
export const updateData = async (_sleepNight) => {
    ...
    const result = await apiPUT('sleepNights', _sleepNight)
...
export const deleteItem = async (option, idToDelete) => {
    ...
    if (option === 'delete') {
        ...
        const result = await apiDELETE('sleepNights', idToDelete)
