...
export default function () {
    ...
    const fetchdata = async () => {
        ...
        const result = await apiGET(`foods/${id}`)
    ...
    return (
        ...
        <div className="container">
            ...
            <div className="alert alert-danger mb-3 mt-3" role="alert" style={{ display: "none" }} id="deleteMessage">
                <h4 className="alert-heading">excluir registro #{food.id}</h4>
...
export const submitForm = (event, food, setFood) => {
    ...
    if (fieldsVerify(food)) {
        ...
        const _food = {
            ...
            caloriesCalculation: food.caloriesCalculation ?? 0,
            comments: food.comments ?? "",
            status: 1,
            created_at: food.created_at ? food.created_at : getDateTime(),
            updated_at: food.updated_at ? food.updated_at : getDateTime(),
        }

        setFood(_food)

        if (food.id && food.id > 0) _food.id = food.id

        if (_food.id && _food.id > 0) updateData(_food)
        else addData(_food)
    }
}
...
export const addData = async (_food) => {
    ...
    const result = await apiPOST('foods', _food)
...
export const updateData = async (_food) => {
    ...
    const result = await apiPUT('foods', _food)
...
export const fieldsVerify = (food) => {
    ...
    if (checkFields) {
        ...
        if (!food.start || food.start.length < 2) {

            document.getElementById('error_message_start').style.display = "block"
            verifyReturn = false
        }

        if (!food.wakeUp || food.wakeUp.length < 2) {
...
export const deleteItem = async (option, idToDelete) => {
    ...
    if (option === 'delete') {
        ...
        const result = await apiDELETE('foods', idToDelete)
